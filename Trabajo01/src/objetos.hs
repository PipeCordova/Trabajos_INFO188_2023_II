...
verificarLava :: (Int, Int) -> [[Celda]] -> Bool
verificarLava (x,y) mapa = case (mapa !! y) !! x of
    Caminable -> True
    Tesoro -> True
    Lava -> True
    Obstaculo -> False
...
agregaObjetos (x, y) objeto mapa = take y mapa ++ [take x (mapa !! y) ++ [objeto] ++ drop (x+1) (mapa !! y)] ++ drop (y+1) mapa
